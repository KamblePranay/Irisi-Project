...
# bulding the user interface
st.set_page_config(page_title='IRIS Projects',layout='wide')

# add title 
st.title("IRIS Project Pranay Kamble")

# Add Inputs For user
sep_len = st.number_input("sepal Lenght: ",min_value=0.00,step=0.01)
...
with open("/workspaces/Irisi-Project/Notebook/pre.pkl", "rb") as file1:
    pre = pickle.load(file1)

# Load the model with pickle
with open('/workspaces/Irisi-Project/Notebook/model.pkl', 'rb') as file2:
    model = pickle.load(file2)

# if submit button is pressed then 
if submit:
    #conver the data into datframe
